...
export function riseWorldPlanet() {
  ...
  wait(3000).then(() => {
    worldPlanet.setAttribute('animation__spin', {
      property: 'rotation',
      from: '0 0 0',
      to: '0 360 0',
      dur: 8000,
...
export function spawnFloatingWords() {
  ...
  words.forEach(({ text, pos, rot, delay }) => {
    setTimeout(() => {
      ...
      entity.setAttribute('animation__drift', {
        property: 'position',
        from: pos,
        to: `${posObj[0]} ${posObj[1] + 1.5} ${posObj[2]}`,
        dur: 5000,
        easing: 'linear'
      })
...
export function traceConstellationPath() {
  ...
  pathPoints.forEach((pos, i) => {
    setTimeout(() => {
      ...
      dot.setAttribute('animation__appear', {
      ...
      dot.setAttribute('animation__pop', {
        property: 'scale',
        from: '0 0 0',
        to: '1 1 1',
        dur: 400,
      ...
      if (i > 0) {
        ...
        line.setAttribute('animation__fadein', {
          property: 'material.opacity',
          from: '0',
          to: '0.5',
          dur: 300,
